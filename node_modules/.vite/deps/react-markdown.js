...
var require_inline_style_parser = /* @__PURE__ */ __commonJS({ "node_modules/inline-style-parser/index.js": ((exports, module) => {
	var COMMENT_REGEX = /\/\*[^*]*\*+([^/*][^*]*\*+)*\//g;
	var NEWLINE_REGEX = /\n/g;
...
function all(values$1, includeImageAlt, includeHtml) {
	...
	const result = [];
	let index$1 = -1;
	while (++index$1 < values$1.length) result[index$1] = one(values$1[index$1], includeImageAlt, includeHtml);
	return result.join("");
}
/**
...
function normalizeUri(value) {
	...
	while (++index$1 < value.length) {
		...
		/** @type {string} */
		let replace = "";
		if (code$1 === codes.percentSign && asciiAlphanumeric(value.charCodeAt(index$1 + 1)) && asciiAlphanumeric(value.charCodeAt(index$1 + 2))) skip = 2;
		else if (code$1 < 128) {
...
function resolveCodeText(events) {
	...
	if ((events[headEnterIndex][1].type === types.lineEnding || events[headEnterIndex][1].type === "space") && (events[tailExitIndex][1].type === types.lineEnding || events[tailExitIndex][1].type === "space")) {
		...
		while (++index$1 < tailExitIndex) if (events[index$1][1].type === types.codeTextData) {
	...
	while (++index$1 <= tailExitIndex) if (enter === void 0) {
		if (index$1 !== tailExitIndex && events[index$1][1].type !== types.lineEnding) enter = index$1;
	} else if (index$1 === tailExitIndex || events[index$1][1].type === types.lineEnding) {
...
function subtokenize(eventsArray) {
	...
	while (++index$1 < events.length) {
		...
		if (index$1 && event[1].type === types.chunkFlow && events.get(index$1 - 1)[1].type === types.listItemPrefix) {
...
function resolveToHtmlFlow(events) {
	...
	while (index$1--) if (events[index$1][0] === "enter" && events[index$1][1].type === types.htmlFlow) break;
	if (index$1 > 1 && events[index$1 - 2][1].type === types.linePrefix) {
...
function resolveToLabelEnd(events, context) {
	...
	while (index$1--) {
		...
		if (open) {
			...
			if (events[index$1][0] === "enter" && token.type === types.labelLink) token._inactive = true;
		} else if (close) {
			if (events[index$1][0] === "enter" && (token.type === types.labelImage || token.type === types.labelLink) && !token._balanced) {
				open = index$1;
		...
		} else if (token.type === types.labelEnd) close = index$1;
	}
...
function tokenizeLabelEnd(effects, ok$2, nok) {
	...
	/** @type {boolean} */
	let defined;
	while (index$1--) if ((self$1.events[index$1][1].type === types.labelImage || self$1.events[index$1][1].type === types.labelLink) && !self$1.events[index$1][1]._balanced) {
...
function resolveToSetextUnderline(events, context) {
	...
	while (index$1--) if (events[index$1][0] === "enter") {
		if (events[index$1][1].type === types.content) {
			content$2 = index$1;
			break;
		}
		if (events[index$1][1].type === types.paragraph) text$4 = index$1;
	} else {
		if (events[index$1][1].type === types.content) events.splice(index$1, 1);
		if (!definition$1 && events[index$1][1].type === types.definition) definition$1 = index$1;
	}
...
function tokenizeSetextUnderline(effects, ok$2, nok) {
	...
	*/
	function start(code$1) {
		...
		while (index$1--) if (self$1.events[index$1][1].type !== types.lineEnding && self$1.events[index$1][1].type !== types.linePrefix && self$1.events[index$1][1].type !== types.content) {
			paragraph$1 = self$1.events[index$1][1].type === types.paragraph;
			break;
		}
...
function createResolver(extraResolver) {
	...
	function resolveAllText(events, context) {
		...
		while (++index$1 <= events.length) if (enter === void 0) {
			if (events[index$1] && events[index$1][1].type === types.data) {
				enter = index$1;
				index$1++;
			}
		} else if (!events[index$1] || events[index$1][1].type !== types.data) {
			if (index$1 !== enter + 2) {
				events[enter][1].end = events[index$1 - 1][1].end;
				events.splice(enter + 2, index$1 - enter - 2);
				index$1 = enter + 2;
...
function compiler(options) {
	...
	*/
	function compile(events) {
		...
		const listStack = [];
		let index$1 = -1;
		while (++index$1 < events.length) if (events[index$1][1].type === types.listOrdered || events[index$1][1].type === types.listUnordered) if (events[index$1][0] === "enter") listStack.push(index$1);
		else {
	...
	function run(...values$1) {
		...
		*/
		function next(error, ...output) {
			...
			while (++index$1 < values$1.length) if (output[index$1] === null || output[index$1] === void 0) output[index$1] = values$1[index$1];
			values$1 = output;
